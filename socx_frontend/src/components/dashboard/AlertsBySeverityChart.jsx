...
import { SEVERITY_COLORS, CHART_THEME } from '../../utils/chartColors'
...
async function fetchSeverityCounts() {
  const results = await Promise.all(
    SEVERITIES.map((severity) => alertsApi.list({ severity, limit: 1 }))
  )
...
export default function AlertsBySeverityChart() {
  ...
  return (
    <WidgetCard title="Alerts by Severity" refreshing={refreshing} lastUpdated={lastUpdated}>
      {loading ? (
