...
class ParseStats:
    ...
    def as_dict(self) -> dict[str, Any]:
        """
        Shape persisted into Log.parse_stats (JSONB). Key names here are
        the public contract other code reads back — e.g. `total_rows`,
        `parsed_rows`, `failed_rows` for the CSV upload/parse flow.
        `skipped_rows` and `sample_errors` are included too since other
        source types (e.g. Linux auth.log) report meaningful skip counts;
        for CSV specifically `skipped_rows` is always 0.
        """
        return {
