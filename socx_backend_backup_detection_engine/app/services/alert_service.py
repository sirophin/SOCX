...
class AlertService:
    ...
    async def acknowledge(
        self, alert_id: uuid.UUID, analyst_id: uuid.UUID, ip_address: Optional[str] = None
    ) -> Alert:
        ...
        await self.alerts.update_status(alert, AlertStatus.INVESTIGATING, analyst_id=analyst_id)
        await self.audit.write(
            action="alert.acknowledge", target_entity="Alert",
            user_id=analyst_id, target_id=alert.id, ip_address=ip_address,
        )
        await self.db.commit()
        await self.db.refresh(alert, attribute_names=["rule", "analyst"])
        return alert

    async def close(
        self, alert_id: uuid.UUID, analyst_id: uuid.UUID, ip_address: Optional[str] = None
    ) -> Alert:
        """Closes the alert: status -> Closed. Whoever is already assigned
        as analyst is left as-is; closing doesn't reassign."""
        alert = await self.alerts.get_by_id(alert_id)
        if alert is None:
            raise AlertNotFoundError(f"Alert {alert_id} not found")
        if alert.status == AlertStatus.CLOSED:
        ...
        await self.alerts.update_status(alert, AlertStatus.CLOSED)
        await self.audit.write(
            action="alert.close", target_entity="Alert",
            user_id=analyst_id, target_id=alert.id, ip_address=ip_address,
        )
        await self.db.commit()
        await self.db.refresh(alert, attribute_names=["rule", "analyst"])
